...
export class AnthropicProvider implements AIProvider {
  ...
  async testConnection(): Promise<{
    ...
    if (!this.isConfigured()) {
      return {
        success: false,
        message: "ANTHROPIC_API_KEY is not set. Add it to Replit Secrets.",
        latencyMs: 0,
      };
    }
    try {
  ...
  async generate(
    ...
    if (!this.isConfigured())
      throw new Error(
        "ANTHROPIC_API_KEY is not configured. Add it to Replit Secrets.",
      );
    ...
    const estimatedCostUsd =
      (usage.input_tokens ?? 0) * costPerInputToken +
      (usage.output_tokens ?? 0) * costPerOutputToken;
