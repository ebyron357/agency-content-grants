...
const SECTION_PATCH_ALLOWED = new Set([
  "title",
  "content",
  "notes",
  "contentFormat",
...
async function getDocumentWithSections(documentId: string) {
  ...
  const sections = await db
    ...
    .where(eq(documentSectionsTable.documentId, documentId));
  return {
    ...doc,
...
router.post("/projects/:id/document", async (req, res): Promise<void> => {
  ...
  const [outline] = await db
    ...
    .limit(1);
  if (outline) {
    const outlineSections = await db
      .select()
      .from(outlineSectionsTable)
      .where(eq(outlineSectionsTable.outlineId, outline.id));
...
router.patch("/document-sections/:id", async (req, res): Promise<void> => {
  ...
  if ((allowed as any).__error) {
    res.status(400).json({ error: (allowed as any).__error });
    return;
  }
  ...
  if (
    (allowed as any).contentFormat !== undefined &&
    !["plain", "html"].includes((allowed as any).contentFormat)
  ) {
    res.status(400).json({ error: "contentFormat must be plain or html" });
    return;
  }
  ...
  if (typeof (allowed as any).content === "string") {
    const requestedFormat =
      (allowed as any).contentFormat ?? section.contentFormat ?? "plain";
    const rawContent =
      requestedFormat === "html"
        ? sanitizeRichHtml((allowed as any).content)
        : (allowed as any).content;
    update.content = rawContent;
    const textForCount =
      requestedFormat === "html" ? stripHtml(rawContent) : rawContent;
    update.wordCount = textForCount.trim()
      ? textForCount.trim().split(/\s+/).length
      : 0;
  } else if ((allowed as any).contentFormat === "html") {
    update.content = sanitizeRichHtml(section.content ?? "");
    const textForCount = stripHtml(update.content);
    update.wordCount = textForCount ? textForCount.split(/\s+/).length : 0;
  }
...
router.post("/document-sections/:id/edit", async (req, res): Promise<void> => {
  ...
  if (!editType) {
    res.status(400).json({ error: "editType is required" });
    return;
  }
  try {
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    logActivity(
      "content_approved",
      `Section "${section.title}" approved${proj ? ` in "${proj.title}"` : ""}`,
      {
...
router.get(
  "/document-sections/:id/revisions",
  async (req, res): Promise<void> => {
    const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
    const section = await getSectionOwned(id, req.session.userId!, res);
    if (!section) return;
    const revisions = await db
      .select()
      .from(sectionRevisionsTable)
      .where(eq(sectionRevisionsTable.documentSectionId, id));
    res.json(revisions.sort((a, b) => b.revisionNumber - a.revisionNumber));
  },
);

router.post(
  "/document-sections/:id/restore",
  async (req, res): Promise<void> => {
    const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
    const section = await getSectionOwned(id, req.session.userId!, res);
    if (!section) return;
    ...
    if (!revisionId) {
      res.status(400).json({ error: "revisionId required" });
      return;
    }
    const [revision] = await db
      .select()
      .from(sectionRevisionsTable)
      .where(eq(sectionRevisionsTable.id, revisionId))
      .limit(1);
    if (!revision) {
...
const imageUpload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    if (ALLOWED_IMAGE_TYPES.has(file.mimetype)) {
      ...
      cb(
        new Error(
          `Unsupported image type: ${file.mimetype}. Allowed: ${[...ALLOWED_IMAGE_TYPES.keys()].join(", ")}`,
        ),
      );
    }
  },
...
const uploadDir =
  process.env.UPLOAD_DIR ?? path.resolve(process.cwd(), "uploads", "images");
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    if (!validateImageMagicBytes(file.buffer, file.mimetype)) {
      res
        .status(400)
        .json({ error: "File content does not match declared image type" });
      return;
    }
    ...
    try {
      ...
      const altText =
        req.body?.altText ?? file.originalname.replace(/\.[^.]+$/, "");
