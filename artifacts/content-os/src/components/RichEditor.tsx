...
import {
  hasRequiredAltText,
  parsePersistedVideoCaption,
  parsePersistedVideoSrc,
} from "@/lib/richEditorMedia";
...
export const VideoEmbed = Node.create({
  ...
  addAttributes() {
    return {
      src: {
        default: null,
        parseHTML: parsePersistedVideoSrc,
      },
      caption: {
        default: "",
        parseHTML: parsePersistedVideoCaption,
      },
    };
  },
...
function Toolbar({
  ...
  const handleImageUpload = async () => {
    ...
    if (!hasRequiredAltText(imageAlt)) {
      setImageError("Alt text is required for accessibility.");
      return;
    }
    ...
    try {
      ...
      form.append("altText", imageAlt.trim());
      form.append("caption", imageCaption);
      const res = await fetch(
        `/api/document-sections/${sectionId}/media/images`,
        {
          method: "POST",
          body: form,
          credentials: "include",
      ...
      editor
        .chain()
        .focus()
        .setImage({ src: data.url, alt: imageAlt.trim(), title: imageCaption })
        .run();
      setImageOpen(false);
      setImageFile(null);
      setImageAlt("");
  ...
  const handleVideoInsert = async () => {
    ...
    try {
      const res = await fetch(
        `/api/document-sections/${sectionId}/media/videos`,
        {
          method: "POST",
