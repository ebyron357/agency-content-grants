...
const CONTENT_TYPES = [
  {
    value: "blog",
    label: "Blog Post",
    description: "Fast, useful, discoverable",
    icon: FileText,
    mode: "Growth",
  },
  {
    value: "article",
    label: "Article",
    description: "A considered editorial piece",
    icon: Newspaper,
    mode: "Editorial",
  },
  {
    value: "guide",
    label: "Guide",
    description: "A clear path from A to B",
    icon: BookOpen,
    mode: "Education",
  },
  {
    value: "manual",
    label: "Manual",
    description: "Structured instructions for complex work",
    icon: BookMarked,
    mode: "Operations",
  },
  {
    value: "whitepaper",
    label: "White Paper",
    description: "Evidence-led perspective",
    icon: StickyNote,
    mode: "Authority",
  },
  {
    value: "newsletter",
    label: "Newsletter",
    description: "A signal worth opening",
    icon: Mail,
    mode: "Retention",
  },
  {
    value: "ebook",
    label: "Ebook",
    description: "A durable content asset",
    icon: BookMarked,
    mode: "Depth",
  },
  {
    value: "report",
    label: "Report",
    description: "Findings with a point of view",
    icon: BarChart3,
    mode: "Intelligence",
  },
  {
    value: "sop",
    label: "SOP / Process",
    description: "Repeatable operational clarity",
    icon: ListChecks,
    mode: "Operations",
  },
] as const;
...
export default function Create() {
  ...
  const [generatingStage, setGeneratingStage] = useState("assignment");
    ...
    pollIntervalRef.current = setInterval(async () => {
      try {
        const res = await fetch(
          `${BASE}/api/projects/${projectId}/workflow-status`,
        );
        if (res.ok)
          setGeneratingStage((await res.json()).currentStage ?? "assignment");
      } catch {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    if (!trimmedTopic || (!brandId && brands.length > 0) || isGenerating)
      return;
    setIsGenerating(true);
    setError(null);
    setGeneratingStage("assignment");
    try {
      const projectBody: Record<string, string> = {
        brandId: brandId || (brands[0]?.id ?? ""),
        title:
          trimmedTopic.length > 100
  ...
  const selected =
    CONTENT_TYPES.find((type) => type.value === contentType) ??
    CONTENT_TYPES[0];
  const canGenerate =
    topic.trim().length > 0 && (!brands.length || brandId) && !isGenerating;
              ...
              <div className="grid gap-5 sm:grid-cols-2">
                <div>
                  ...
                  {brands.length > 0 ? (
                    ...
                    <div className="rounded-xl border border-amber-400/20 bg-amber-400/[0.06] px-3 py-2.5 text-xs text-amber-200">
                      No brands yet.{" "}
